...
interface WoCScriptPubKey {
  addresses?: string[]
}

interface WoCTxOutput {
  value: number
  scriptPubKey?: WoCScriptPubKey
}

interface WoCScriptSig {
  addresses?: string[]
}

interface WoCTxInput {
  scriptSig?: WoCScriptSig
}
...
export async function verifyTransaction(opts: VerifyOptions): Promise<VerifyResult> {
  ...
  try {
    ...
    const outputs: WoCTxOutput[] = tx.vout || []
    ...
    const inputs: WoCTxInput[] = tx.vin || []
